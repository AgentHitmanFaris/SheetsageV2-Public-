...
def transcribe_audio_piano(audio_file, progress=gr.Progress()):
    ...
    try:
        ...
        base_output_dir = pathlib.Path(
            current_config.get("output_dir", os.path.join(os.getcwd(), "output"))
        )
        ...
        synthesized_audio_path = synthesize_midi(
            str(output_midi_path),
            soundfont_path,
            output_dir,
            filename="piano_synth.wav",
        )
        if synthesized_audio_path:
            ...
            mixed_audio_path, original_audio_segment_path = create_mix(
                audio_file, synthesized_audio_path, output_dir
            )
            if mixed_audio_path:
        ...
        if original_audio_segment_path or audio_file:
            tracks_dict["Original"] = cache_file_for_playback(
                original_audio_segment_path
        ...
        return format_player_output(
            output_files_list, None, tracks_dict, "Transcription successful!"
        )
...
def transcribe_audio_drums(audio_file, progress=gr.Progress()):
    ...
    try:
        ...
        output_midi_path = run_omnizart(
            audio_file, str(output_dir), mode="drum", callback=omni_callback
        )
        ...
        return format_player_output(
            output_files_list, None, tracks_dict, "Drum Transcription successful!"
        )
...
def transcribe_audio_omnizart_advanced(audio_file, mode, progress=gr.Progress()):
    ...
    try:
        ...
        output_path = run_omnizart(
            audio_file, str(output_dir), mode=mode, callback=omni_callback
        )
        ...
        return format_player_output(
            ...
            None,
            tracks_dict,
            f"Omnizart ({mode}) Transcription successful!",
        )
...
def transcribe_audio_lunaverus(audio_file, progress=gr.Progress()):
    ...
    try:
        ...
        weights_path = os.path.join(
            os.path.dirname(__file__), "modules", "lunaverus_weights.pth"
        )
        ...
        output_midi_path = run_lunaverus(
            audio_file, model_weights_path=weights_path, device=device
        )
        ...
        synthesized_audio_path = synthesize_midi(
            str(final_midi_path), soundfont_path, output_dir, filename="v3_synth.wav"
        )
        if synthesized_audio_path:
            output_files_list.append(synthesized_audio_path)
        ...
        return format_player_output(
            ...
            None,
            tracks_dict,
            "SheetSage V3 Transcription successful!",
        )
...
def load_history_project(selected_item):
    ...
    try:
        ...
        base_output_dir = current_config.get(
        ...
        try:
            ...
            logging.warning(
                f"Security Alert: Path traversal attempt blocked. {abs_project} is not in {abs_base}"
            )
        ...
        download_files = [
            f
            for f in files
            if f.lower().endswith((".pdf", ".midi", ".mid", ".ly", ".wav"))
        ]
        ...
        if not orig_path:
            ...
            orig_path = next(
                (f for f in files if "original" in f and f.endswith(".wav")), None
            )
        ...
        synth_path = next(
            (f for f in files if "synth" in f and f.endswith(".wav")), None
        )
        ...
        vocals_path = next(
            (f for f in files if "vocals" in f and f.endswith(".wav")), None
        )

        # 3. Piano Roll
        fig = None
        midi_path = next(
            (f for f in download_files if f.endswith(".midi") or f.endswith(".mid")),
            None,
        )
        if midi_path:
            try:
                ...
                if all_notes:
                    ...
                    ax.barh(
                        pitches, durations, left=starts, height=0.8, color="#4A90E2"
                    )
        ...
        tracks_dict = {}
        if orig_path:
            tracks_dict["Original"] = cache_file_for_playback(orig_path)
        if mix_path:
            tracks_dict["Mixed"] = cache_file_for_playback(mix_path)
        if synth_path:
            tracks_dict["Synthesized"] = cache_file_for_playback(synth_path)
        ...
        return format_player_output(
            download_files, fig, tracks_dict, f"Loaded project: {rel_path}"
        )
...
def synthesize_midi(
    midi_path, soundfont_path_config, output_dir, filename="output.wav"
):
    """
    ...
    try:
        ...
        if total_notes == 0 or duration <= 0:
            logging.warning(
                "MIDI file has no notes or zero duration. Skipping synthesis."
            )
            return None
...
def create_mix(original_path, synth_path, output_dir, offset=0.0, duration=None):
    ...
    try:
        ...
        y_orig, _ = librosa.load(
            original_path, sr=44100, offset=offset, duration=duration, mono=True
        )
...
def transcribe_audio_lead_sheet(
    ...
    try:
        ...
        if not audio_path_or_url:
            return format_player_output(
                None, None, {}, "Please provide an audio file or URL."
            )
        ...
        if separate_vocals and audio_file:
            try:
                ...
                python_exe = (
                    ...
                    if os.path.isdir("python_embeded")
                    else "python"
                )
                ...
                cmd_args = [
                    python_exe,
                    os.path.join("sheetsage", "run_demucs.py"),
                    "-n",
                    "htdemucs",
                    "-o",
                    str(sep_out_dir),
                ...
                if proc.returncode != 0:
                    ...
                    if vocals_path.exists():
                        ...
                        if ht_dir.exists():
                            try:
                                files = [
                                    str(p.relative_to(ht_dir))
                                    for p in ht_dir.glob("**/*")
                                ]
        ...
        segment_start_hint = (
            float(segment_start_hint) if segment_start_hint is not None else None
        )
        segment_end_hint = (
            float(segment_end_hint) if segment_end_hint is not None else None
        )
        ...
        if segment_end_hint is not None and segment_end_hint <= start_val:
            logging.warning(
                f"Ignoring invalid segment_end_hint ({segment_end_hint}) <= start ({start_val})"
            )
        ...
        def timer_loop():
            while not stop_event.is_set():
                ...
                if elapsed > 0 and elapsed % 2 == 0:
                    print(
                        f"[SheetSage Timer] Elapsed: {elapsed}s...",
                        end="\r",
                        flush=True,
                    )
                time.sleep(1)
        ...
        result_tuple = sheetsage(
            ...
            beats_per_measure_hint=(
                int(beats_per_measure) if beats_per_measure else None
            ),
            beats_per_minute_hint=(
                int(beats_per_minute_hint) if beats_per_minute_hint else None
            ),
            detect_melody=detect_melody,
            detect_harmony=detect_harmony,
            melody_threshold=(
                float(melody_threshold) if melody_threshold is not None else None
            ),
            harmony_threshold=(
                float(harmony_threshold) if harmony_threshold is not None else None
            ),
        ...
        if separate_vocals:
            print(
                "TIP: If separation succeeded, check the 'output/demucs' folder for the separated vocal stems. You can use those stems for manual refinement if needed."
            )
        ...
        if midi_path_str:
            synthesized_audio_path = synthesize_midi(
                midi_path_str, current_config["soundfont_path"], output_dir
            )
            if synthesized_audio_path:
                output_files_list.append(synthesized_audio_path)
        ...
        if demucs_vocals_path and os.path.exists(demucs_vocals_path):
            ...
            for stem in ["bass.wav", "drums.wav", "other.wav"]:
                ...
                if os.path.exists(stem_path):
                    tracks_dict[stem.replace(".wav", "").capitalize()] = (
                        cache_file_for_playback(stem_path)
                    )
...
def generate_mixer_html(orig_path, synth_path, vocals_path=None):
    ...
    if src_orig:
        tracks_dom += (
            f'<audio id="{player_id}_orig" src="{src_orig}" preload="auto"></audio>'
        )
        controls_html += mk_track_row(
            "Original", f"{player_id}_orig", f"{player_id}_vol_orig", "#3b82f6"
        )  # Blue
    ...
    if src_synth:
        tracks_dom += (
            f'<audio id="{player_id}_synth" src="{src_synth}" preload="auto"></audio>'
        )
        controls_html += mk_track_row(
            "Transcribe", f"{player_id}_synth", f"{player_id}_vol_synth", "#eab308"
        )  # Yellow
    ...
    if src_vocals:
        tracks_dom += (
            f'<audio id="{player_id}_vocals" src="{src_vocals}" preload="auto"></audio>'
        )
        controls_html += mk_track_row(
            "Vocals", f"{player_id}_vocals", f"{player_id}_vol_vocals", "#ef4444"
        )  # Red
...
def transcribe_audio_basic_pitch(
    ...
    try:
        ...
        if separate_vocals and audio_file:
            try:
                msg_demucs = (
                    "Separating vocals with Demucs (this may take a few minutes)..."
                )
                print(msg_demucs)
                progress(0.1, desc=msg_demucs)
        ...
        cmd = [
            ...
            script_path,
            "--audio_path",
            str(target_audio_path),  # Use target (vocals or original)
            "--output_midi_path",
            str(output_midi_path),
            "--measures_per_chunk",
            str(measures_per_chunk),
        ]
        ...
        midi_file = next(
            (f for f in generated_files if "raw" in pathlib.Path(f).name), None
        )
        if not midi_file:
            midi_file = next(
                (
        ...
        if midi_file:
            ...
            synthesized_audio_path = synthesize_midi(
                midi_file,
                current_config["soundfont_path"],
                output_dir,
                filename="basic_pitch_synth.wav",
            )
            if synthesized_audio_path:
                output_files_list.append(synthesized_audio_path)
...
with gr.Blocks(title="Sheet Sage") as demo:
    ...
    with gr.Tabs():
        with gr.TabItem("Transcribe"):
            with gr.Row(elem_classes=["container"]):
                with gr.Column(scale=1):
                    ...
                    with gr.Tabs():
                        ...
                        with gr.TabItem("Audio URL"):
                            audio_url = gr.Textbox(
                                label="URL", placeholder="https://example.com/audio.mp3"
                            )
                    ...
                    with gr.Group(visible=True) as shared_options:
                        ...
                        with gr.Row():
                            separate_vocals = gr.Checkbox(
                                ...
                                value=True,
                                info="Recommended for songs with vocals.",
                            )
                            generate_pdf = gr.Checkbox(
                                label="Generate Sheet Music PDF",
                                value=True,
                                info="If unchecked, skips PDF formatting (Faster).",
                            )
                    ...
                    with gr.Row():
                        submit_btn = gr.Button(
                            "✨ Transcribe", variant="primary", size="lg", scale=2
                        )
                        cancel_btn = gr.Button(
                            "🚫 Cancel Task", variant="stop", scale=1
                        )

                    with gr.Row():
                        restart_btn = gr.Button(
                            "🔄 Restart App", variant="secondary", scale=1
                        )
                        stop_app_btn = gr.Button("⛔ Stop App", variant="stop", scale=1)

                    # Visibility Logic
                    def update_visibility(selected_mode):
                        # Shared: Visible for Lead Sheet OR Basic Pitch OR Drums
                        show_shared = selected_mode in [
                            "Lead Sheet (Standard)",
                            "Basic Pitch (Polyphonic)",
                            "Drums (Omnizart)",
                        ]
                ...
                with gr.Column(scale=1):
                    ...
                    output_files = gr.Files(
                        ...
                        visible=True,
                        file_types=[".pdf", ".midi", ".ly", ".wav"],
                    )

                    with gr.Accordion("Preview", open=True):
                        piano_roll_plot = gr.Plot(label="Piano Roll Visualization")
                        with gr.Row():
                            transcribe_track_selector = gr.Dropdown(
                                label="Select Track",
                                choices=[],
                                interactive=True,
                                scale=1,
                            )
                            transcribe_main_player = gr.Audio(
                                label="Audio Player",
                                type="filepath",
                                interactive=False,
                                scale=3,
            ...
            submit_event = submit_btn.click(
                ...
                outputs=[
                    output_files,
                    piano_roll_plot,
                    transcribe_tracks_state,
                    status_msg,
                    transcribe_track_selector,
                    transcribe_main_player,
                ],
            )
        ...
        with gr.TabItem("Omnizart (Advanced)"):
            ...
            gr.Markdown(
                "Use the full capabilities of Omnizart for various transcription tasks."
            )
            ...
            with gr.Row(elem_classes=["container"]):
                with gr.Column(scale=1):
                    ...
                    omni_desc = gr.Markdown(
                        value="**Music**: General polyphonic transcription (e.g., Piano)."
                    )
                ...
                with gr.Column(scale=1):
                    ...
                    with gr.Group():
                        plot_omni_adv = gr.Plot(
                            label="Piano Roll", visible=False
                        )  # Helper for structure
                        player_omni_adv = gr.Audio(
                            label="Audio Player", interactive=False
                        )
                        track_sel_omni_adv = gr.Dropdown(
                            label="Select Track", choices=[], interactive=True
                        )
        ...
        with gr.TabItem("History"):
            ...
            gr.Markdown(
                "Select a previous transcription to reload its results (Downloads, Mixer, Piano Roll)."
            )
            ...
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    with gr.Row():
                        hist_track_selector = gr.Dropdown(
                            label="Select Track", choices=[], interactive=True, scale=1
                        )
                        hist_main_player = gr.Audio(
                            label="Audio Player",
                            type="filepath",
                            interactive=False,
                            scale=3,
        ...
        with gr.TabItem("Settings"):
            ...
            save_settings_btn.click(
                save_settings, inputs=[sf_path_input], outputs=[settings_status]
            )
...
if __name__ == "__main__":
    ...
    demo.launch(
        server_name="0.0.0.0", server_port=7860, share=False, allowed_paths=allowed
    )
