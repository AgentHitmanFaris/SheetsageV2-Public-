...
from sheetsage.utils import run_cmd_sync
...
def theorytab_find_applicable(timed_events, search_event, eps=1e-3):
    """
    Finds the event in `timed_events` that is applicable at the time of `search_event`.

    Args:
        timed_events (list): List of events with a 'beat' attribute.
        search_event (dict): Event to search for.
        eps (float): Epsilon tolerance.

    Returns:
        dict: The applicable event.

    Raises:
        ValueError: If no applicable event is found.
    """
...
def estimate_key_changes(meter_changes, harmony, melody):
    """
    Estimates the key of a piece using Melisma key finding algorithm.
    ...
    Returns:
        KeyChanges: Estimated key changes.

    Raises:
        Exception: If key estimation fails.
    """
...
%change_penalty = 0.002
    ...
    f_fd, f_name = tempfile.mkstemp()
    p_fd, p_name = tempfile.mkstemp()
    try:
        with os.fdopen(f_fd, "w") as f:
            f.write(formatted)
        with os.fdopen(p_fd, "w") as p:
            p.write(parameters)
        res, stdout, stderr = run_cmd_sync(
            f"melisma-key -p {p_name} {f_name}", timeout=60
        )
