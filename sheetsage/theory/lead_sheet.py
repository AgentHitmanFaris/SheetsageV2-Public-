...
class LeadSheet(_ImmutableIterable):
    """
    Represents a complete lead sheet including melody, harmony, meter, tempo, and key changes.
    """
    def __new__(
        cls,
        meter_changes,
        ...
        """
        Creates a new LeadSheet.

        Args:
            meter_changes (list): List of meter changes.
            tempo_changes (list): List of tempo changes.
            key_changes (list): List of key changes.
            harmony (list): List of chords.
            melody (list): List of melody notes.
            total_num_tertiary (int, optional): Total duration in tertiary units.

        Raises:
            TypeError: If input types are incorrect.
            ValueError: If input values are invalid or out of range.
            NotImplementedError: If multiple meter changes are provided (not yet supported).
        """
        # Run value checks
        ...
        # Ensure all changes start on downbeats
        tertiary_per_group = int(np.prod(meter))
        for l in [meter_changes, tempo_changes, key_changes]:
            for o, _ in l:
                if o % tertiary_per_group != 0:
                    raise ValueError(f"Change at onset {o} is not on a downbeat (multiple of {tertiary_per_group})")

        # Harmony can be loose
        harmony_fixed = []
        for o, c in harmony:
             if o % tertiary_per_group != 0:
                  logging.warning(f"Harmony change at onset {o} is not on a downbeat (multiple of {tertiary_per_group}). Snapping to nearest downbeat.")
                  # Snap to nearest downbeat
                  o = round(o / tertiary_per_group) * tertiary_per_group
             harmony_fixed.append((o, c))
        # Deduplicate if snapping caused collisions
        if harmony_fixed:
            harmony_unique = []
            seen_onsets = set()
            # If multiple chords snap to same onset, take the last one (assuming it overrides)
            # or keep first? Let's sort by onset and keep last.
            harmony_fixed.sort(key=lambda x: x[0])

            last_o = -1
            for o, c in harmony_fixed:
                 if o == last_o:
                      # Replace the previous chord at this onset
                      harmony_unique[-1] = (o, c)
                 else:
                      harmony_unique.append((o, c))
                 last_o = o
            harmony = Harmony(*harmony_unique)
    ...
    def as_lily(
        ...
        """
        Generates LilyPond source code for the lead sheet.

        Args:
            clef (str): The clef to use ('treble' or 'bass').
            adjust_melody_octave (bool): Whether to automatically adjust the melody octave.
            skip_unknown_chords (bool): Whether to skip chords that cannot be converted to LilyPond format.
            artist (str, optional): Artist name for the header.
            title (str, optional): Title for the header.

        Returns:
            str: The LilyPond source code.

        Raises:
            ValueError: If clef is invalid or if unknown chords are encountered (and not skipped).
            NotImplementedError: If unsupported features (like multiple tempo/key changes) are present.
        """
    ...
    def as_midi(self, pulse_to_time_fn=None, adjust_melody_octave=True):
        """
        Generates a MIDI file (bytes) from the lead sheet.

        Args:
            pulse_to_time_fn (callable, optional): Function to convert pulse indices to time.
            adjust_melody_octave (bool): Whether to adjust melody octave.

        Returns:
            bytes: The content of the MIDI file.

        Raises:
            NotImplementedError: If tempo changes are complex and no pulse_to_time_fn is provided.
        """
        (
            meter_changes,
            tempo_changes,
        ...
        # Create harmony (Piano - Left Hand / Bass)
        harmony_ins = pretty_midi.Instrument(program=0, name="Harmony (Left Hand)")  # Piano
        for i, (t, c) in enumerate(harmony):
            if i + 1 < len(harmony):
                d = harmony[i + 1][0] - t
            else:
                d = total_num_tertiary - t
            # Use octave 3 for bass accompaniment
            for p in c.as_midi_pitches(octave_0=3):
                harmony_ins.notes.append(
                    pretty_midi.Note(
                        70, # Lower velocity for accompaniment
                        p,
        ...
        # Create melody (Piano - Right Hand / Treble)
        melody_ins = pretty_midi.Instrument(program=0, name="Melody (Right Hand)") # Piano
        for t, d, ns in melody:
            melody_ins.notes.append(
                pretty_midi.Note(
                    100, # Higher velocity for melody
                    ns.as_midi_pitch(),
                    tertiary_to_time_fn(t),
                    tertiary_to_time_fn(t + d),
                )
        ...
        midi.instruments.extend([melody_ins, harmony_ins])
        temp_path = os.path.join(tempfile.gettempdir(), f"sheetsage_midi_{uuid.uuid4().hex}.mid")
        try:
    ...
    def from_theorytab(
        ...
        """
        Creates a LeadSheet from a TheoryTab analysis dictionary.

        Args:
            analysis (dict): TheoryTab analysis data.
            ignore_inversion (bool): If True, treats all chords as root position.
            skip_bad_notes_and_chords (bool): If True, skips invalid notes/chords instead of raising error.

        Returns:
            LeadSheet: The constructed lead sheet.

        Raises:
            TheorytabValueError: If invalid data is encountered and not skipped.
            ValueError: If items extend beyond the track end.
        """
