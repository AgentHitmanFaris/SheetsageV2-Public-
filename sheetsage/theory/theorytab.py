...
class TheorytabValueError(ValueError):
    """Exception raised for invalid TheoryTab values."""
    pass


class TheorytabMeter(_TheorytabDict):
    """Represents a TheoryTab meter."""
...
class _TheorytabNoteOrChord(_TheorytabDict):
    ...
    def will_sound(self, min_duration=1e-8):
        """Checks if the note/chord will sound (is not rest and has duration)."""
        return (
...
class TheorytabNote(_TheorytabNoteOrChord):
    ...
    def as_note(self, key, legacy_behavior=False):
        ...
        Args:
            key (Key or TheorytabKey): The key context.
            legacy_behavior (bool): If True, uses legacy accidental parsing.

        Returns:
            Note: The converted note.
        """
...
class TheorytabChord(_TheorytabNoteOrChord):
    ...
    def as_chord(self, key, root_position=False):
        """
        Converts to a Chord object.

        Args:
            key (Key or TheorytabKey): The key context.
            root_position (bool): If True, returns the chord in root position.

        Returns:
            Chord: The converted chord.
        """
        if isinstance(key, dict) and not isinstance(key, TheorytabKey):
            key = TheorytabKey(key)
        if isinstance(key, TheorytabKey):
        ...
        if self.will_sound():
            ...
            for alt in chord["alterations"]:
                base_d = int(alt[1:])
                found = False
                for d in [base_d, base_d + 7]:
                    if d in chord_degree_to_interval:
