...
class TestModules(unittest.TestCase):
    ...
    def test_transformer_decoder(self):
        class EmbeddingTransformerDecoder(TransformerDecoder):
            ...
            def forward(self, src, src_len, tgt, tgt_len):
                ...
                src_enc = src
                src_enc = src_enc.reshape(src_max_len * batch_size, -1)
    ...
    def test_ctc_transducer(self):
        with torch.no_grad():
            ...
            self.assertAlmostEqual(torch.abs(out).sum().item(), 231966.6, delta=0.5)
    ...
    def test_s2s_transducer(self):
        with torch.no_grad():
            ...
            loss = F.cross_entropy(
                out[:-1].reshape((self.tgt_max_len - 1) * self.batch_size, -1),
                self.tgt[1:].reshape((self.tgt_max_len - 1) * self.batch_size),
                ignore_index=-1,
                reduction="mean",
            )
