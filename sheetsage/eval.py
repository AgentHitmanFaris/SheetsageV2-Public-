...
def _trim_midi(midi, segment_start, segment_end, tolerance=0):
    """
    Trims a MIDI object to include only notes within the specified segment.

    Args:
        midi (pretty_midi.PrettyMIDI): The MIDI object to trim.
        segment_start (float): The start time of the segment.
        segment_end (float): The end time of the segment.
        tolerance (float): Tolerance in seconds to extend the segment boundaries.

    Returns:
        tuple: A tuple containing the trimmed MIDI object and the number of dropped notes.
    """
...
def _midi_to_mir_eval(midi, dummy_offsets=True):
    """
    Converts a MIDI object to the format required by mir_eval.

    Args:
        midi (pretty_midi.PrettyMIDI): The MIDI object to convert.
        dummy_offsets (bool): If True, creates dummy offsets if not present.

    Returns:
        tuple: A tuple containing intervals (np.ndarray) and pitches (np.ndarray).
    """
    notes = []
    for i in midi.instruments:
        if i.is_drum:
...
def _mir_eval_onset_prf(
    ...
    """
    Computes precision, recall, and F1 score for onset detection using mir_eval.

    Args:
        ref_intervals (np.ndarray): Reference intervals.
        ref_pitches (np.ndarray): Reference pitches.
        est_intervals (np.ndarray): Estimated intervals.
        est_pitches (np.ndarray): Estimated pitches.
        tolerance (float): Onset tolerance in seconds.
...
def f1(
    ...
    """
    Computes the F1 score between reference and estimated MIDI data, accounting for octave shifts.

    Args:
        ref_midi (pretty_midi.PrettyMIDI): Reference MIDI.
        est_midi (pretty_midi.PrettyMIDI): Estimated MIDI.
        tolerance (float): Onset tolerance for evaluation.
        octave_invariant_radius (int): Maximum octave shift to check for.

    Returns:
        tuple: Best F1 score, precision, recall, and the best octave shift found.
    """
    ...
    f1s = []
    for o in octaves:
        p, r, f1_val = _mir_eval_onset_prf(
            ref_intervals,
            (o * 12) + ref_pitches,
            est_intervals,
            est_pitches,
...
def eval_dataset(ref, est, allow_abstain=False, return_detail=False):
    """
    Evaluates a dataset of estimated transcriptions against reference transcriptions.

    Args:
        ref (str or pathlib.Path): Path to the reference directory or archive.
        est (str or pathlib.Path): Path to the estimated directory or archive.
        allow_abstain (bool): If True, allows missing estimates without error (counts as abstained).
        return_detail (bool): If True, returns detailed results for each example.

    Returns:
        float or tuple: The mean F1 score, or (mean F1 score, detailed results) if return_detail is True.

    Raises:
        Exception: If directories are not found or abstaining is not allowed but occurs.
    """
