# Portable GPU Setup Guide for atoscore
...
1. Verify that a folder named `cuda_libs` exists in the root of your `atoscore_Core` directory.
   - Path: `D:\Document\atoscore\atoscore_Core\cuda_libs`
...
4. Copy the following files into your `atoscore_Core\cuda_libs` folder:
   - `cudart64_110.dll`
   - `cublas64_11.dll`
   - `cublasLt64_11.dll`
   - `cufft64_10.dll`
...
4. Copy the following file into your `atoscore_Core\cuda_libs` folder:
   - `cudnn64_8.dll`
...
**Your folder `atoscore_Core\cuda_libs` should look like this:**
```text
cuda_libs/
├── cublas64_11.dll
├── cublasLt64_11.dll
