...
from ...infer import Status as atoscoreStatus
from ...infer import atoscore
...
class FetchAudioError(JobError):
    """Error raised when audio retrieval fails."""
    pass


class BulkyAudioError(JobError):
    """Error raised when audio is too large or too long."""
    pass
...
def _work(wid):
    ...
    while True:
        ...
        def status_change_callback(s):
            ...
            assert isinstance(s, JobStatus) or isinstance(s, atoscoreStatus)
            _JOB_STATUS[jid] = s
...
def submit():
    ...
    Expects a JSON payload or form data with job parameters.

    Returns:
        dict: A dictionary containing the job ID ('jid'), cache status ('cached'),
              and queue position ('position').
    """
    # Check payload size
...
def heartbeat(jid):
    ...
    Returns:
        json: The status name of the job.
    """
    status = _JOB_STATUS.get(jid)
    if status is None:
...
def download(jid):
    """
    Downloads the result of a job (the lead sheet JSON).

    Args:
        jid (str): Job ID.

    Returns:
        file: The lead sheet JSON file.
    """
