...
from atoscore.infer import atoscore, Status
from atoscore.theory.internal import Melody, Note
from atoscore.theory import LeadSheet
...
def transcribe_basic_pitch(audio_path, output_midi_path,
    ...
    """
    Transcribes audio to Lead Sheet using Basic Pitch for melody and AtoScore for harmony/grid.
    ...
    ss_start = time.time()
    try:
        # NOTE: We disable the Gradio tqdm here (pass lambda x: x) because generally atoscore 
    ...
    target_temp_base = os.environ.get("ATOSCORE_TEMP", os.environ.get("TEMP", None))
    ...
    # However, atoscore() returns segment_beats_times which are just beats.
    # We need the full tertiary grid or a way to map to it.
    
    # atoscore.infer.atoscore returns (lead_sheet, segment_beats, segment_beats_times)
    ...
    # atoscore return: segment_beats are indices relative to start?
    ...
    try:
        ...
        # Actually atoscore provides create_beat_to_time_fn.
